...
<form action="<?= BASE_URL ?>/products/save<?= $product ? '/' . $product['id'] : '' ?>" method="POST" enctype="multipart/form-data" id="productForm">
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6" x-data="{ isOut: <?= ($product && $product['is_out_of_stock']) ? 'true' : 'false' ?> }">
        
        <!-- Left Column: Main Form -->
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-surface rounded-lg border border-border shadow-sm p-6 space-y-6 w-full">
        ...
        <div class="space-y-6">
            <!-- Image Panel -->
            <div class="bg-surface rounded-lg border border-border shadow-sm p-6 space-y-4">
                <h3 class="text-base font-bold text-textPrimary">Gambar Produk</h3>
                
                <!-- Upload Box / Preview -->
                <div class="relative flex flex-col items-center justify-center p-4 border-2 border-dashed border-border rounded-xl bg-background hover:bg-gray-50 transition-colors duration-200 text-center overflow-hidden min-h-[160px]">
                    
                    <?php if ($product && !empty($product['image_url'])): ?>
                        <img id="product-img-preview" src="<?= htmlspecialchars($product['image_url']) ?>" alt="Preview Produk" class="max-h-36 max-w-full object-contain rounded-lg">
                        <div id="product-img-placeholder" class="hidden flex flex-col items-center justify-center">
                            ...
                            <span class="text-xs text-textSecondary font-medium">Klik tombol di bawah untuk pilih gambar</span>
                        </div>
                    <?php else: ?>
                        <img id="product-img-preview" src="" alt="Preview Produk" class="hidden max-h-36 max-w-full object-contain rounded-lg">
                        <div id="product-img-placeholder" class="flex flex-col items-center justify-center">
                            <svg class="h-10 w-10 text-textSecondary mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                            </svg>
                            <span class="text-sm text-textSecondary font-medium">Belum ada gambar</span>
                            <span class="text-xs text-textSecondary mt-1">PNG, JPG, WEBP (Max 2MB)</span>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <!-- Upload Button -->
                <div class="space-y-2">
                    <label class="w-full flex items-center justify-center gap-2 px-4 py-2 bg-surface text-primary border border-primary hover:bg-primary-light rounded-lg text-xs font-bold transition-colors cursor-pointer shadow-sm">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/>
                        </svg>
                        <span><?= ($product && !empty($product['image_url'])) ? 'Ganti Gambar' : 'Pilih File Gambar' ?></span>
                        <input type="file" name="image" id="product_image_input" accept="image/png, image/jpeg, image/webp" class="hidden" onchange="previewProductImageFile(this)">
                    </label>

                    <?php if ($product && !empty($product['image_url'])): ?>
                        <label class="flex items-center gap-2 text-xs text-danger cursor-pointer pt-1">
                            <input type="checkbox" name="remove_image" value="1" class="rounded text-danger focus:ring-danger">
                            <span>Hapus Gambar Produk</span>
                        </label>
                    <?php endif; ?>
                </div>
            </div>
